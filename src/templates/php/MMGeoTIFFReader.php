...
//        $csvFname = str_replace(".tif",$timestamp.".csv",$fileName);
        ...
        $Rearth = 63781.37; // units of 100m, to get area in hectares
        $area = 0;
        for ($iNode=0;$iNode<$polyPoints-1;$iNode++)
        {
        	 $area += ( deg2rad($lonarr[$iNode+1]) - deg2rad($lonarr[$iNode])) * ( 2 + sin(deg2rad($latarr[$iNode])) + sin(deg2rad($latarr[$iNode+1])));
        }
        $area += ( deg2rad($lonarr[0]) - deg2rad($lonarr[$polyPoints-1])) * ( 2 + sin(deg2rad($latarr[$polyPoints-1])) + sin(deg2rad($latarr[0])));

        $area = $area * $Rearth * $Rearth / 2;
        if ($area<0) $area = -$area;
...
//        
        ...
        //$msg = intval($area*10)/10 . ',' . intval($polysum) . ',' . intval($polystdev) . ','. intval($polymin). ',' . intval($polymax) ;
